...
#This file performs a simple linear regression
...
ran_seed = 20 #our seed to randomize data
...
#For the final CSV when we use just one train size and one ft set:
forecast_prediction = []
nrmse_t_final = []
nrmse_v_final = []
skill_t_final = []
...
for hp in hor_pred:
    if hp.endswith("min"):
        hor_pred_indices = int(int(hp.replace('min','')) * 60 / tg) #amount of indices we'll move. 5 because it's our time granularity.
    if hp.endswith("s"):
...
#TRAIN SIZE:
    ...
    for ts in train_size:
        ...
        x_t = x_train.iloc[y_t_index_valid] #like our randomization but just picking the same indices
...
#STATIONS TO SELECT:
        ...
        for ft in feature_values:
                ...
                for n in range(len(ft)):
                    ...
                    for i in range(lencol):
                        ...
                        if x.columns[i].startswith(ft[n]):
                            ...
                            X_t = pd.concat([X_t,x_t[x.columns[i]]],axis=1,ignore_index=True)
            ...
            output_text = '/linear_regression_stations_' + sts + 'for_' + hp + '_prediction_horizon_' + str(prcnt) + '_train_size'
 
            linreg = LinearRegression()
            print('Fitting '+output_text+'\n')
            fit_start = time.time()
            linreg.fit(X_t,y_t)
            fit_end = time.time()
            ...
            joblib.dump(linreg, model_filename)

            print('Predicting...\n')
            y_pred_train = linreg.predict(X_t)
            print('Validating...\n')
            y_pred_val = linreg.predict(X_val)
            print('Getting scores\n')
            scr = linreg.score(X_t,y_t)
            scr_val = linreg.score(X_val,y_val)
            scrs.append(scr)
            ...
            rmse_val = (np.mean((y_pred_val - y_val) **2)) ** 0.5  #our "custom" score for validation
            rmse_train = (np.mean((y_pred_train - y_t) **2)) ** 0.5 #our "custom" score for training
...
#This is just for use with ONE different train size and feature values set:        
#fcp = pd.DataFrame(forecast_prediction)        
...
total_scores.to_csv(csvs_folder + '/scores_report.csv',header=True,index=False)
