...
#This file creates the trained models for a given neural network configuration for best and worst features
...
def addOptions(parser):
   parser.add_option("--NN_best_worst", default="",
...
if not options.NN_best_worst:
   print >> sys.stderr, "No configuration file specified\n"
   sys.exit(1)

#with open('config.json', 'r') as cfg_file:
with open(options.NN_best_worst, 'r') as cfg_file:
...
seed = cfg_data['seed']
best_worst_folder = cfg_data['best_worst_folder']
numfeatures = cfg_data['numfeatures']
...
for hp in hor_pred:
    ...
    best_worst_root = best_worst_folder + 'best_worst_' + str(numfeatures) + '_features_' + hp + '.csv'
    best_worst_file = pd.read_csv(best_worst_root)
...
#STATIONS TO SELECT:
        ...
        for i in range(len(best_worst_file.columns)):

            stations = best_worst_file.columns[i]
            X_t = pd.DataFrame()
            X_val = pd.DataFrame()
            
            X_t = x_t[best_worst_file[stations]]
            X_val = x_val[best_worst_file[stations]]
            ...
            for av in alpha_values:
                ...
                output_text = '/' + stations + '_' + str(numfeatures) + '_features_for_' + hp + '_prediction_horizon_' + str(prcnt) + '_train_size_' + len_hls + '_hidden_layers_with_' + hls_neurons_str + 'neurons'
