...
#This file performs a random forest regression and saves the models and metrics
...
#TRAIN SIZE:
    ...
    for ts in train_size:
        ...
        y_t = y_train.sample(n_rows,random_state=seed_rfr)
...
#STATIONS TO SELECT:
        ...
        for ft in feature_values:
            ...
            print('Predicting...\n')
            y_pred_train = regr.predict(X_t)
            print('Validating...\n')
            y_pred_val = regr.predict(X_val)
            print('Getting scores\n')
            scr = regr.score(X_t,y_t)
            scr_val = regr.score(X_val,y_val)
            scrs.append(scr)
...
total_scores.to_csv(csvs_folder + '/scores_report_random_forest.csv',header=True,index=False)
