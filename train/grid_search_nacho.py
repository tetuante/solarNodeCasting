...
class GridSearch(object):
    ...
    def explore(self):
        ...
        for normalized_csv_path in self.normalized_csv_paths:
                ...
                for target in target_distances:
                    ...
                    for estimator in self.estimator_configs:
                        ...
                        clf = GridSearchCV(model, tuned_parameters, cv=3, \
                                            scoring=estimator["scoring"], n_jobs=20)
			...
			# PROBAMOS EL PREDICT
			x_pred = clf.predict(x_set)
			modelo = clf.best_estimator_
			_params_ = modelo.get_params(deep=True)
			#_params_ = clf.get_params()
			...
			###################
                        ...
                        destination_path = os.path.join(self.results_destination_folder, \
                                                        estimator["model"] + ".csv")
			filename = normalized_csv_path.replace("data/","")
			filename = filename.replace(".csv","")
			prediction_path = os.path.join(self.results_destination_folder, \
							filename + "_" + str(k) +  "_prediccion.csv")
        		with open("coeficientes", "a") as cfile:
				cfile.write("PARAMS: {} {}\n".format(modelo.coef_,modelo.intercept_))			
